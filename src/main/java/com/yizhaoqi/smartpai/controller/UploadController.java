...
public class UploadController {
    ...
    public ResponseEntity<Map<String, Object>> uploadChunk(
        ...
        if (orgTag == null || orgTag.isEmpty()) {
            ...
            List<Integer> uploadedChunks = uploadService.getUploadedChunks(fileMd5, userId);//获取已上传分片列表
            int actualTotalChunks = uploadService.getTotalChunks(fileMd5, userId);//获取实际分片总数
    ...
    public ResponseEntity<Map<String, Object>> mergeFile(
        ...
        try {
            ...
            // 创建任务     构建文件处理任务对象，用于提交到 Kafka 进行异步处理
            //             * 包含文件的所有必要信息，以便消费者服务能够完整处理文件
