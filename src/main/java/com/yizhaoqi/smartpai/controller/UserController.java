...
public class UserController {
    ...
    // 验证用户身份并生成JWT令牌
    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody UserRequest request) { //@RequestBody 表示把请求体 JSON 反序列化成 UserRequest。
        // UserRequest 是 record：record UserRequest(String username, String password) {}
        //也就是前端要传：{ "username": "...", "password": "..." }
        //开启一次性能/耗时监控，用于统计登录接口耗时与结果。
        //一般最后会 monitor.end(...) 记录结束原因或结果。
        ...
        try {
            if (request.username() == null || request.username().isEmpty() ||
                ...
                return ResponseEntity.badRequest().body(Map.of("code", 400, "message", "Username and password cannot be empty"));
                //ResponseEntity.badRequest() 等价于返回 `HTTP 400`。
                //.body(Map.of(...))：返回一个 Map，Spring 会把它序列化成 JSON（如 `{"code":400,"message":"..."}`）。
            }
            //userService.authenticateUser(...) 负责“校验账号密码是否正确”。
            //设计上它返回 username（成功）或 null（失败）。
            String username = userService.authenticateUser(request.username(), request.password());
            //失败则返回 `HTTP 401`（未认证 / 凭证无效）。
            //同时记录登录失败原因（凭证错误）
            if (username == null) {
            ...
            //成功则生成 JWT 令牌
            String token = jwtUtils.generateToken(username);//`generateToken`：生成 JWT 令牌
            String refreshToken = jwtUtils.generateRefreshToken(username);//`generateRefreshToken`：生成刷新令牌//
            LogUtils.logUserOperation(username, "LOGIN", "token_generation", "SUCCESS");//`logUserOperation`：记录用户操作
            monitor.end("登录成功");
