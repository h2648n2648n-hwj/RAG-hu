    ...
    public List<SearchResult> searchWithPermission(String query, String userId, int topK) {
        ...
        try {
            ...
            SearchResponse<EsDocument> response = esClient.search(s -> {
                        s.index("knowledge_base"); //指定要搜索的 ES 索引名称
                        // KNN 召回
                        int recallK = topK * 30; // KNN 召回窗口
                        s.knn(kn -> kn     //基于向量空间距离寻找最相似的文档这里使用的是 ES 的 Approximate k-NN 功能
                                .field("vector")
                                .queryVector(queryVector) // ：queryVector 是前面通过 embedToVectorList(query) 生成的
                                .k(recallK)  //  期望返回的最近邻数量
                                .numCandidates(recallK)  // 每个分片（shard）考虑的候选文档数量
                        );
                        // 必须命中关键词 + 权限过滤
                        s.query(q -> q.bool(b -> b
                                .must(mst -> mst.match(m -> m.field("textContent").query(query))) //field("textContent") 匹配文档的文本内容字段 //.query(query) 使用用户的原始查询文本
                                .filter(f -> f.bool(bf -> bf   //OR 逻辑（至少满足一个 should 条件）
                                        // 条件1: 用户可访问自己的文档
                                        .should(s1 -> s1.term(t -> t.field("userId").value(userDbId)))
                                        // 条件2: 公开文档
                                        .should(s2 -> s2.term(t -> t.field("public").value(true)))
                                        // 条件3: 组织标签
                                        .should(s3 -> {  //这部分根据用户标签数量动态生成不同的查询结构
                                            if (userEffectiveTags.isEmpty()) {
                                                return s3.matchNone(mn -> mn);  //matchNone 查询  当用户没有任何组织标签时，这个条件永远不生效
                                            } else if (userEffectiveTags.size() == 1) {  //只有一个标签 直接使用简单的 term 查询
